...
// Página protegida para exportação estática: exibe mensagem amigável se não for client-side
const isStaticExport = typeof window === "undefined";

export default function AdminFormFieldsPage() {
  if (isStaticExport) {
    return (
      <div className="max-w-xl mx-auto mt-16 p-8 text-center border rounded bg-background text-muted-foreground">
        <h2 className="text-2xl font-bold mb-4">Área restrita</h2>
        <p>Esta página só pode ser acessada em ambiente dinâmico (admin).<br />No deploy estático, ela não está disponível.</p>
      </div>
    );
  }

  // ...existing code...
  // (Coloque aqui o código do FormFieldsManager)
