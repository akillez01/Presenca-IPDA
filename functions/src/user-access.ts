...
export const NAVIGATION_PERMISSION_OPTIONS: PermissionOption[] = [
  { permission: 'dashboard', label: 'Dashboard', description: 'Painel inicial e visão geral do sistema.' },
  {
  ...
  { permission: 'reports', label: 'Relatórios', description: 'Indicadores, totais e relatórios operacionais.' },
  { permission: 'scanner', label: 'Scanner QR Code', description: 'Tela de leitura e registro por QR Code.' },
  { permission: 'register', label: 'Cadastrar Membros', description: 'Formulário principal de cadastro de membros.' },
  { permission: 'baptism', label: 'Batismo', description: 'Dashboard e registros específicos de batismo.' },
  { permission: 'sedeEstadual', label: 'Sede Estadual', description: 'Cadastro de membros da Sede Estadual.' },
  {
    permission: 'letters',
    label: 'Cartas',
    description: 'Rotinas de carta de recomendação e documentos correlatos.',
  },
  { permission: 'config', label: 'Configurações', description: 'Acesso à área administrativa de configurações.' },
  {
...
export function normalizeNavigationPermissions(
  ...
  return NAVIGATION_PERMISSION_OPTIONS.map((option) => option.permission).filter((permission) =>
...
export function deriveCapabilityFlags(permissions: NavigationPermission[]) {
  ...
  return {
    ...
    canViewAttendance:
      permissionSet.has('presencadecadastrados') || permissionSet.has('reports') || permissionSet.has('dashboard'),
