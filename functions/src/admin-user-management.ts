import { getAuth, type UpdateRequest, type UserRecord } from 'firebase-admin/auth';
...
function buildManagedUserRecord(params: {
  ...
  const claimRole =
    typeof authUser?.customClaims?.role === 'string' ? (authUser.customClaims.role as ManagedUserRole) : undefined;
  ...
  const storedPermissions = Array.isArray(profile?.permissions)
    ...
    : Array.isArray(authUser?.customClaims?.permissions)
      ? authUser.customClaims.permissions.filter((value: unknown): value is string => typeof value === 'string')
      : undefined;
  ...
  return {
    uid,
    email: authUser?.email || (typeof profile?.email === 'string' ? profile.email : ''),
    displayName:
...
export async function resolveAdminAccess(decodedToken: {
  uid: string;
  email?: string;
  permissions?: unknown;
...
export async function createManagedUser(
  ...
  if (existingAuthUser) {
    ...
    await adminAuth.setCustomUserClaims(existingAuthUser.uid, buildClaims(input.accessProfile, input.permissions, true));
...
export async function updateManagedUser(input: UpdateManagedUserInput, adminAccess: AdminAccessContext) {
  ...
  if (authUser) {
    ...
    await adminAuth.setCustomUserClaims(input.uid, buildClaims(input.accessProfile, input.permissions, input.active));
  }
...
export async function deleteManagedUser(uid: string, adminAccess: AdminAccessContext) {
  ...
  try {
    ...
    authUserType = typeof authUser.customClaims?.userType === 'string' ? authUser.customClaims.userType : undefined;

    if (
      isImmutableAdminAccount({
        email: authEmail,
        role: authRole || (typeof profile.role === 'string' ? profile.role : undefined),
        accessProfile: getAccessProfileFromRole(typeof profile.role === 'string' ? profile.role : authRole),
